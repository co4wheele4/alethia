...
describe('useExtraction', () => {
  ...
  it('handles acceptSuggestion', async () => {
    const client = createMockClient('AcceptSuggestion', { acceptSuggestion: { id: 's1', status: 'ACCEPTED' } });
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <ApolloProvider client={client}>{children}</ApolloProvider>
    );
  ...
  it('handles rejectSuggestion', async () => {
    const client = createMockClient('RejectSuggestion', { rejectSuggestion: { id: 's1', status: 'REJECTED' } });
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <ApolloProvider client={client}>{children}</ApolloProvider>
    );
