...
function isDomainArray(name: string): boolean {
  const lower = name.toLowerCase();
  return DOMAIN_IDENTIFIERS.some((id) => lower.includes(id));
}

function getArrayVarName(node: Rule.Node): string | null {
...
export const noInferenceLogic: Rule.RuleModule = {
  meta: {
    type: 'problem',
    docs: {
    ...
    messages: {
      sortForbidden:
        "Derived semantics are forbidden (ADR-022). sort() on claims/evidence implies ranking.",
      reduceForbidden:
        "Derived semantics are forbidden (ADR-022). reduce() on domain objects may produce computed metrics.",
      mathMaxMin:
        "Derived semantics are forbidden (ADR-022). Math.max/Math.min on domain objects implies comparison.",
    },
  },
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (callee.type === 'MemberExpression') {
          ...
          if (prop.type === 'Identifier') {
            if (prop.name === 'sort') {
              const varName = getArrayVarName(callee as Rule.Node);
