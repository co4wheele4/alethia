// test/e2e/helpers/setup-e2e.ts
import { config } from 'dotenv';
import { resolve } from 'path';
import { execSync } from 'node:child_process';

// Load .env.test if it exists, otherwise load .env but override with test database
// From root level, backend .env.test is in aletheia-backend directory
const envTestPath = resolve(process.cwd(), 'aletheia-backend', '.env.test');
const backendEnvPath = resolve(process.cwd(), 'aletheia-backend', '.env');
try {
  // Try to load .env.test first
  config({ path: envTestPath });
} catch {
  // If .env.test doesn't exist, try backend .env
  try {
...
if (!globalThis.__ALETHEIA_E2E_MIGRATED__) {
  ...
  execSync('npx prisma generate', {
    stdio: 'inherit',
    env: process.env,
    cwd: resolve(process.cwd(), 'aletheia-backend'),
  });
